...
static map<string, int> caozuo = {{"L.D",0},{"ADD.D",1},{"SUB.D" ,2},{"MUL.D",3},{"DIV.D",4} };
...
public:
	...
	void simclear()
	{
		...
		RS.resize(rssize, ReservationStation());
        vector<ReservationStation>::iterator it;
        it=RS.begin();
        while(it!=RS.end())
        {
            it->clear();
        ...
        RegS.resize(resize, RegisterStation());
        vector<RegisterStation>::iterator rgit;
        rgit=RegS.begin();
        while(rgit!=RegS.end())
        {
            rgit->q=-1;
	...
	void insload(ifstream& ops)
	{
		...
		for(;;)
		{
            ...
            inst->runtime = time[caozuo[inst->ins.name]];
			inst->state = 0;
			IS.push_back(*inst);
		}
		ops.close();
